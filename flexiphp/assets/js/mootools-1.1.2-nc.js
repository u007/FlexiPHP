...
Element.extend({
	...
	*/
	...
	clone: function(contents){
		var el = mooTool(this.cloneNode(contents !== false));
	...
	*/
	...
	getParent: function(){
		return mooTool(this.parentNode);
	},
...
function $E(selector, filter){
	return (mooTool(filter) || document).getElement(selector);
};
...
function $ES(selector, filter){
	return (mooTool(filter) || document).getElementsBySelector(selector);
};
...
Element.Methods.Dom = {
	...
	*/
	...
	getElement: function(selector){
		return mooTool(this.getElements(selector, true)[0] || false);
	},
	...
	add: function(fn){
		...
		if (document.readyState && window.webkit){
			...
			if (!mooTool('ie_ready')){
				...
				mooTool('ie_ready').onreadystatechange = function(){
...
Element.extend({
	...
	Example:
		>var myEffects = mooTool(myElement).effects({duration: 1000, transition: Fx.Transitions.Sine.easeInOut});
...
Fx.Scroll = Fx.Base.extend({
	...
	*/
	...
	toElement: function(el){
		...
		var target = mooTool(el).getPosition(this.options.overflown);
...
Drag.Base = new Class({
	...
	initialize: function(el, options){
		this.setOptions(options);
		this.element = mooTool(el);
		this.handle = mooTool(this.options.handle) || this.element;
		this.mouse = {'now': {}, 'pos': {}};
...
Drag.Move = Drag.Base.extend({
	...
	initialize: function(el, options){
		...
		this.container = mooTool(this.options.container);
...
var Ajax = XHR.extend({
	...
	onComplete: function(){
		if (this.options.update) mooTool(this.options.update).empty().setHTML(this.response.text);
	...
	*/
	...
	request: function(data){
		...
		switch($type(data)){
			case 'element': data = mooTool(data).toQueryString(); break;
...
var Scroller = new Class({
	...
	initialize: function(element, options){
		this.setOptions(options);
		this.element = mooTool(element);
		this.mousemover = ([window, document].contains(element)) ? mooTool(document.body) : this.element;
	},
...
var SmoothScroll = Fx.Scroll.extend({
	...
	initialize: function(options){
		...
		this.links.each(function(link){
			...
			if (anchor && mooTool(anchor)) this.useLink(link, anchor);
		}, this);
		if (!window.webkit419) this.addEvent('onComplete', function(){
...
var Accordion = Fx.Elements.extend({
	...
	initialize: function(){
		...
		$each(arguments, function(argument, i){
			switch($type(argument)){
				...
				case 'element': container = mooTool(argument); break;
				default:
					var temp = $$(argument);
		...
		this.elements = elements || [];
		this.container = mooTool(container);
		this.setOptions(options);
		this.previous = -1;
	...
	*/
	...
	addSection: function(toggler, element, pos){
		toggler = mooTool(toggler);
		element = mooTool(element);
