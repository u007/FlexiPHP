...
$iWidth = isset($vars["#width"]) ? $vars["#width"] : 300;
$iHeight = isset($vars["#height"]) ? $vars["#height"] : 300;

$sLabel = empty($vars["#label"]) ? " - " : $vars["#label"];
?>
...
<div class="flexiphp_div_input">
	<input type="hidden" name="<?=$vars["#name"]?>" <?=is_null($mValue) ? "" : " value=\"" . $mValue . "\""?><?=$sMaxLen?>
  ...
  <div id="<?=$vars["#id"]?>_display" class="flexiphp_div_display"><?=$sLabel?></div>
  <div style="float: left;"><a id="<?=$vars["#id"]?>_select">Select</a></div>
	<br/>
  ...
  <script type="text/javascript">
    jQuery(document).ready(function() {
      jQuery('#<?=$vars["#id"]?>_select').colorbox({iframe:true, href:"<?=$vars["#url"]?>",
        width: <?=$iWidth?>, height: <?=$iHeight?>,
          onOpen: function() {
            _targetPickElement = {fieldvalue:"#<?=$vars["#id"]?>", fieldlabel:"#<?=$vars["#id"]?>_display"}
          }
        });
    })
  </script>
