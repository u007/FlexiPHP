...
class WebCalendar
{
    ...
    */
    ...
    {
			return "javascript:showsidecal(" . $month . "," . $year . ");";
    }
    ...
    */
    function getYearHTML($year)
    {
		...
		function adjustDay($aDay, $aMonth, $aYear)
    {
			$aYearMonth = $this->adjustDate($aMonth, $aYear);
			$iMonth 	= $aYearMonth[0];
			$iYear		= $aYearMonth[1];
			
			$iDay = $aDay;
			//adjust until day is positive
			while($iDay < 1)
			{
				$iDay+=1;
				
				$iMonth -=1;
				$aYearMonth = $this->adjustDate($iMonth, $iYear);
				$iMonth 	= $aYearMonth[0];
				$iYear		= $aYearMonth[1];
			
				$iDay += $this->daysInMonth[$iMonth-1];
			}
			
			//adjust until day is within the month days range
			while($iDay > $this->daysInMonth[$iMonth-1])
			{
				$iDay -= $this->daysInMonth[$iMonth-1];
